...
def index(request):
    ...
    if opt == "gettoken":
        ...
        paramDict = {k: v[0] for k, v in dict(paramDict).items()}
        paramDict["testLevel"] = testLevel
        paramDict["retMsg"] = retMsg
        paramDict["langDict"] = langDict
        paramDict["addBy"] = addBy
        paramDict["addByName"] = addByName
        paramDict["addByObj"] = addByObj
        paramDict["httpConfKey"] = httpConfKey
        paramDict["httpConfKeyAlias"] = httpConfKeyAlias
        paramDict["httpConfKeyObj"] = httpConfKeyObj
        paramDict["isSendEmail"] = isSendEmail
        paramDict["isSaveHistory"] = isSaveHistory
        paramDict["execPlatform"] = execPlatform
        paramDict["token"] = token
        paramDict["timeout"] = timeout
        paramDict["host"] = host
        paramDict["version"] = version
...
def runtask(request,paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
    addByName = paramDict["addByName"]
    addByObj = paramDict["addByObj"]
    httpConfKey = paramDict["httpConfKey"]
    httpConfKeyAlias = paramDict["httpConfKeyAlias"]
    httpConfKeyObj = paramDict["httpConfKeyObj"]
    isSendEmail = paramDict["isSendEmail"]
    isSaveHistory = paramDict["isSaveHistory"]
    execPlatform = paramDict["execPlatform"]
    ...
    taskExecuteData.testLevel = paramDict.get("testLevel", 100)
    ...
    if retApiReturn.code != ApiReturn.CODE_OK :
        ...
        return getTaskResult(token,taskExecuteId,timeout, paramDict)
...
def getTaskResult(token,taskExecuteId,timeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def runinterface(request,paramDict):
    ...
    try:
        ...
        if retApiReturn.code != ApiReturn.CODE_OK:
            ...
            return getInterfaceResult(token, testDebugId, timeout, paramDict)
        # else:
...
def callbackinterface(request, paramDict):
    ...
    return getInterfaceResult(token,interfaceDebugId,timeout, paramDict)


def getInterfaceResult(token,interfaceDebugId,execTimeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def runtestcase(request,paramDict):
    ...
    if retApiReturn.code != ApiReturn.CODE_OK:
        ...
        #执行成功，获取结果
        return getCaseResult(token, testCaseDebugId,testCaseStepDebugId, timeout, paramDict)
...
def callbacktestcase(request, paramDict):
    ...
    return getCaseResult(token, caseDebugId,testCaseStepDebugId, timeout, paramDict)


def getCaseResult(token,testCaseDebugId,testCaseStepDebugId,execTimeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def runuitask(request, paramDict):
    ...
    if retApiReturn.code != ApiReturn.CODE_OK:
        ...
        return getuiTaskResult(token, taskExecuteId, timeout, paramDict)
...
def getuiTaskResult(token,taskExecuteId,timeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def rundubbotask(request,paramDict):
    ...
    if retApiReturn.code != ApiReturn.CODE_OK :
        ...
        return getdubboTaskResult(token,taskExecuteId,timeout,paramDict)
...
def getdubboTaskResult(token,taskExecuteId,timeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def runtasksuite(request,paramDict):
    ...
    return getTaskSuiteResult(token, taskSuiteExecuteModel.id, timeout, paramDict)


def getTaskSuiteResult(token,taskSuiteExecuteId,timeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
    ...
    while True:
        ...
        resDict = dbModelToDict(runRes)
        if runRes.execStatus == 3:
            if isinstance(resDict['testResultMsg'], dict):
                ...
                return HttpResponse(
                    ApiReturn(ApiReturn.CODE_TASK_EXECUTE_EXCEPTION, "返回的testResultMsg不是可序列化的json： %s" % resDict['testResultMsg'],
                              resDict).toJson())
...
def callbacktasksuite(request, paramDict):
    ...
    return getTaskSuiteResult(token,taskSuiteExecuteId,timeout, paramDict)
...
def rundubbotasksuite(request,paramDict):
    ...
    addUserLog(request, "invoke->runtasksuite->成功", "PASS")
    return getDubboTaskSuiteResult(token, taskSuiteExecuteModel.id, timeout, paramDict)


def getDubboTaskSuiteResult(token,taskSuiteExecuteId,timeout, paramDict):
    # 反向解析相关参数
    retMsg = paramDict["retMsg"]
    langDict = paramDict["langDict"]
    addBy = paramDict["addBy"]
...
def callbackdubbotasksuite(request, paramDict):
    ...
    return getDubboTaskSuiteResult(token,taskSuiteExecuteId,timeout, paramDict)
