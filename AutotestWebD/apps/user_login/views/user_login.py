...
def userLogin(request):
    ...
    if confDict['COMMON']['loginType'] == "LDAP" and LDAP_ENABLED:
        #先执行LDAP登录
        logger.debug("#############TESTFORLDAP--NEW#######################")
        logger.debug("%s：%s  开始ldap登录" % (userName, pwd))
        user = auth.authenticate(username=userName, password=pwd)
        logger.debug(user)
        logger.debug(type(user))
        logger.debug("#############TESTFORLDAP--END#########22##############")
    ...
    elif confDict['COMMON']['loginType'] == "SSO":
        # 这是实现自己公司的SSO登录
        pass
