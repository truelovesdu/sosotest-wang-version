...
function dataInit() {
    ...
    if (optionFromContext != "add" && optionFromContext != "addbymock") {
        ...
        try{
            ...
            if (urlList.length > 1){
                ...
                for(var index = 0 ;index < paramList.length;index ++){
                    if ($("#commonValBeforeInput").val() === ""){
                        $("#commonValBeforeInput").val("# python\nparam_" + paramList[index][0] +"= \""+ decodeURIComponent(paramList[index][1]).replace('"', '\\"').trim()+"\"");
                    }else{
            ...
            var body_formdata_index = 1;
            var body_urlencode_index = 2;
            var body_raw_index = 3;
            ...
            if(contentTypeFromHeader.startsWith("multipart/form-data")){
                ...
                if(paramsData.hasOwnProperty("postData")){
                    ...
                    if (mimeType.startsWith("multipart/form-data")){
                        ...
                        $("input:radio[name='body']").eq(body_formdata_index).click();
                        $("input:radio[name='body']").eq(body_formdata_index).click();
                        ...
                        for(var index = 0;index < paramsFromFormdata.length;index ++){
                            if ($("#commonValBeforeInput").val() === ""){
                                $("#commonValBeforeInput").val("# python\nbody_" + paramsFromFormdata[index]["name"] +"= \""+ decodeURIComponent(paramsFromFormdata[index]["value"]).replace('"', '\\"').trim()+"\"");
                            }else{
                                $("#commonValBeforeInput").val($("#commonValBeforeInput").val() + "\nbody_" + paramsFromFormdata[index]["name"] +"= \""+ decodeURIComponent(paramsFromFormdata[index]["value"]).replace('"', '\\"').trim()+"\"");
                            }
                ...
                if(paramsData.hasOwnProperty("postData")){
                    ...
                    if (mimeType.indexOf("x-www-form-urlencoded")>-1 && prePostData.hasOwnProperty("params")){
                        //x-www-form-urlencoded   key value模式的
                        $("input:radio[name='body']").eq(body_urlencode_index).click();
                        var postData = prePostData["params"];
                        var paramsList = [];
                        for(var index = 0;index < postData.length;index ++){
                            if ($("#commonValBeforeInput").val() === ""){
                                $("#commonValBeforeInput").val("# python\nbody_" + postData[index]["name"] +"= \""+ decodeURIComponent(postData[index]["value"]).replace('"', '\\"').trim()+"\"");
                            }else{
                                $("#commonValBeforeInput").val($("#commonValBeforeInput").val() + "\nbody_" + postData[index]["name"] +"= \""+ decodeURIComponent(postData[index]["value"]).replace('"', '\\"').trim()+"\"");
                            }
                        ...
                        $("input:radio[name='body']").eq(body_raw_index).click();
            ...
            if(paramsData.hasOwnProperty("response")){
                $("#commonValAfterInput").val("# python\nasserts('"+paramsData["response"].replace("'", "\\'")+"')");
            }
        }catch (e){
            alert(e);
