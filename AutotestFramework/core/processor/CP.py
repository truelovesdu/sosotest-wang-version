...
class CP(object):
    ...
    def process_TEXT_subStr(context,strTobeProcessed):
        ...
        #对于获取到的文本进行一次处理。
        # 判断textValue是不是python模式，默认python模式
        tagStr = textValue.split("\n")[0].strip()
        if tagStr.startswith("#") and tagStr.replace("#", "").replace(";","").strip() == context.keywordModeStrPrefix:
            context.generateVarsKeyListAndVarsPool(textValue)  # DONE 处理完varsPre，生成对应的varsPool
        else:
            BP.process_python_code(textValue, context)
        return resValue
