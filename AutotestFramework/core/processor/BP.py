            ...
            class TimeLimited(Thread):
                ...
                def run(self):
                    with stdoutIO() as sout:
                        try:
                            print("[%s]开始执行..." % context.interfaceId)
                            exec(finalCode, context.varsPool)
                        except Exception as eInner:
                            if isinstance(eInner, MyAssertError):
                                print("终止执行，测试结果[%s]: %s" % (eInner.testResult, eInner.retMsg.strip()))
                            elif isinstance(eInner, AssertionError):
                                # 使用 assert False, msg来进行了断言
                                context.setFAIL("assert False, %s" % str(eInner))
                                print("终止执行，assert False。%s" % (str(eInner)))
                            elif str(eInner) == "BREAK":
                                print("<ERROR:PYTHON MODE 下，用户BREAK代码执行！>")
                            else:
                                print(traceback.format_exc())
                                context.setERROR(traceback.format_exc())
                    context.pythonModeExecuteLog = sout.getvalue()
